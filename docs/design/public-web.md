...
- **M8 — consumer parity pass (from the audit) — building.** Web-own design (desktop-creative + responsive, Planity-informed, brand identity). **M8.1 ✅** discovery home (`/`) + `/recherche` — hero service+commune search → existing landings/`/recherche`; category tiles; Salons populaires; **"Partout à Abidjan"** commune×category directory → the SEO landings; value props; FAQ; WebSite+SearchAction & FAQPage JSON-LD; on-brand SVG hero. No backend change. 12 unit + 3 e2e. **Closes G1.** Spec: [web-m8-1-discovery.md](web-m8-1-discovery.md). · **M8.2** provider extras (map · Avant/Après · artistes) · **M8.3** account extras (rebook · avis · favoris).
