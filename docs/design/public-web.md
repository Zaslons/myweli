...
| **Status** | **Feature-complete (M0–M8).** Public SEO + consumer + pro web all built & CI-green; at flow-parity with both apps. **Deploys at the accounts phase.** |
...
- **M7 — pro web dashboard** (desktop-optimised; several PRs). Spec: [web-m7-pro-dashboard.md](web-m7-pro-dashboard.md).
  ...
  - **M7.3 catalogue/dispo/profil/abonnement — building** (split): **7.3a ✅** Catalogue : Services (`/pro/catalogue` list + inline create/edit/delete; list reuses `GET /me/provider`; mutations via pro BFF, backend-enforced ownership; 6 unit + 1 e2e — spec [web-m7-3-catalogue.md](web-m7-3-catalogue.md)) · **7.3b ✅** Catalogue : Équipe (Services|Équipe tabs; artistes list + inline CRUD; per-staff hours deferred; 2 unit + 1 e2e — spec [web-m7-3b-equipe.md](web-m7-3b-equipe.md)) · **7.3c ✅** Disponibilités (`/pro/disponibilites` hours + tampon + dates bloquées; load reuses `GET /me/provider`, save via pro-BFF PUT; multi-slot/pause round-tripped; 3 unit + 1 e2e — spec [web-m7-3c-dispo.md](web-m7-3c-dispo.md)) · **7.3d ✅** Abonnement (`/pro/abonnement` read-only PRO-SUB) + **revenue cards on `/pro`** (consume `GET /providers/{id}/dashboard` — **closes G3**); 4 unit + 1 e2e — spec [web-m7-3d-abo-stats.md](web-m7-3d-abo-stats.md) · **7.3e Profil + Acompte + Médias** (split): **7.3e-i ✅** editable Profil (new backend `PATCH /providers/{id}`, threat T30) + Acompte (deposit policy) + home nudge; backend +2, web +8 unit + 1 e2e — spec [web-m7-3e-profil.md](web-m7-3e-profil.md) · **7.3e-ii ✅** Médias (`/pro/medias` Photos grid reorder/remove + **full browser upload** via `/uploads/sign`→R2 + Avant/Après pairs; 5 unit + 1 e2e — spec [web-m7-3e-ii-medias.md](web-m7-3e-ii-medias.md)). **→ M7 complete; pro dashboard at parity.**
- **M8 — consumer parity pass (from the audit) — building.** Web-own design (desktop-creative + responsive, Planity-informed, brand identity). **M8.1 ✅** discovery home (`/`) + `/recherche` — hero service+commune search → existing landings/`/recherche`; category tiles; Salons populaires; **"Partout à Abidjan"** commune×category directory → the SEO landings; value props; FAQ; WebSite+SearchAction & FAQPage JSON-LD; on-brand SVG hero. No backend change. 12 unit + 3 e2e. **Closes G1.** Spec: [web-m8-1-discovery.md](web-m8-1-discovery.md). · **M8.2 ✅** provider page extras — **Avant/Après** + **Galerie** + interactive **OpenStreetMap** embed + **two-column desktop layout with a sticky booking panel** (mobile: fixed bottom bar); no backend change; 3 unit + 1 e2e; **closes G2** ([web-m8-2-provider.md](web-m8-2-provider.md)) · **M8.3 ✅** account extras — **rebook** + **laisser un avis** (completed bookings) + **favoris** (account section + provider-page heart); no backend change; 3 unit + 3 e2e; **closes G4** ([web-m8-3-account.md](web-m8-3-account.md)). **→ M8 complete; the whole web epic (M0–M8) is feature-complete and at parity with both apps. Deploys at the accounts phase.**
