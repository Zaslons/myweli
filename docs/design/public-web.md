...
- **M4 — SEO landing `/<categorie>-<commune>` (✅ done):** via the `app/[slug]` dispatcher (provider-first → landing → 404); provider cards + internal-link chips + `ItemList`/`Breadcrumb`/`FAQPage` JSON-LD; empty→`noindex`; landings in the sitemap. Category×commune (service landings = M4.1). Spec: [web-m4-landing.md](web-m4-landing.md).
