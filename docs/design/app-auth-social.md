...
- **iOS:** ~~`Info.plist` gains `GIDClientID` (the iOS client ID) + the **reversed client ID** URL scheme~~ — stale since #499, and the literal it named was the **consumer's** client. One `Info.plist` is shared by both flavours, so hardcoding a client there put the consumer's in the Pro build, which Google rejects against `com.myweli.pro` (store rule **2.1**) — and it failed silently, because `google_sign_in_ios` resolves the id from the bundled `GoogleService-Info.plist` first and only falls back to `GIDClientID`; Pro's config had no `CLIENT_ID`, so it fell through to a value that was *wrong* rather than *missing*. The URL scheme has no fallback at all. `Info.plist` now names neither: `GIDClientID` is `$(GOOGLE_CLIENT_ID)` (`ios/Runner/Info.plist:42`) and the scheme `$(GOOGLE_REVERSED_CLIENT_ID)` (`:32`), both **per-configuration build settings** written by `ios/tool/setup_flavours.rb` from the `google_client_id` column of its `FLAVOURS` table (`:51`, `:56` → `:110`) — consumer `…ah75c2b…`, pro `…o68qiuiv…`. Same mechanism `$(APP_DISPLAY_NAME)` already used, which is why it was chosen over the xcconfigs (the script rewrites those on every run). `ios_google_client_test.dart` pins that the two flavours carry **different** clients and that `Info.plist` hardcodes none.
