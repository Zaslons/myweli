...
- **P4 — Consumer icon + native splash + open animation** ✅ — client `AppIcon.appiconset` (iOS, white-on-black) + Android adaptive mipmaps (`ic_launcher` + round + monochrome + notification `ic_stat`) + manifest `roundIcon`; `flutter_native_splash` generates the native launch screen (`#000`, white mark; `web: false` so the admin keeps its P2 template); `/splash` now plays the **`loader_v2` (mixed_white)** open animation on brand-black → `/home`. `flutter analyze` 0 · 338 tests green. Icon/splash visuals verify on a device build.
