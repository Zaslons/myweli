...
- **P1 — Web branding** (`web/`, myweli-web-guardrails + WEB-DESIGN-STANDARDS): favicons + `site.webmanifest` + apple-touch + android-chrome + maskable + OG → `web/public`; wire Next `metadata` (icons/manifest/openGraph); replace `logo.svg` with the **wordmark/lockup SVG**; header renders the SVG. Keep e2e/Lighthouse green.
