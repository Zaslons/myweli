...
| 28 | Two tab strips can push the page sideways, and the overflow gate cannot see them (§9) | **2 surfaces** | found by A11's mobile census, and recorded in the MOBILE register (SYSTEM.md row 54) — which is the wrong register for a web defect, so it is here too. `components/pro/RendezVousClient.tsx:308` and `components/account/AccountClient.tsx:269` render the same French labels as mobile's converted tab bars (`LIST_TABS` = « Aujourd'hui »/« À venir »/« En attente »/« Tous », and « À venir »/« Passés »/« Annulés »), both as a plain `flex gap-s border-b border-divider` with **no `flex-wrap` and no `overflow-x`**. A `<button>` flex item's `min-width: auto` means it cannot shrink below its text, so the row pushes the page sideways rather than clipping. **And `type-overflow.spec.ts` iterates `PUBLIC_ROUTES` only**, so `/pro/*` and `/mon-compte` are outside the matrix and nothing measures either. Note the divergence this exposes: web's *earnings* twin (`RevenusClient.tsx`, which calls itself "the app's earnings_screen, web-adapted") solved the same problem with `flex-wrap`, while mobile now scrolls — two surfaces, two answers, one claim of parity. Computed at a 375px viewport, **not measured in a browser** | *new — extend the overflow matrix first, then decide wrap vs scroll* |
