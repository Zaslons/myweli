...
**The brand is written `MyWeli`** — capital M, capital W, everywhere a human reads
it: labels, body copy, notification channels, legal documents, store listings.
Not `Myweli`, not `MYWELI`, not `myweli`. Identifiers in code (`MyweliMonthGrid`)
and infrastructure keys (`myweli_default`, `myweli-uploads`) are exempt — they are
not read as the brand. §21 row 89 is what happens without this line.
...
Flutter 3.44.9) therefore *is* the authority: the goldens run inside the existing
...
| 88 | **Four surfaces gave no touch feedback at all — the ripple painted behind the card it was inside** | **4 surfaces · every row on them** | found by the Flutter 3.44.9 upgrade, and by nothing else that could have. A `ListTile` paints its ink splash on its nearest `Material` ANCESTOR, so a coloured `Container`/`DecoratedBox` between the two hides it — the ripple renders **underneath the very surface the row sits on**. The commune picker, the salon picker, the notification-preferences card and the deposit-settings switch were all built that way, so tapping a row on any of them looked like nothing happened. **Nothing here could have caught it, and that is the point.** §12 asks for feedback on interaction, but the ripple is Material's own and no rule in this document claims it; a golden captures a *static* frame, so a splash that never paints looks the same as one that has not been triggered; and no a11y helper walks ink. It took Flutter shipping `list_tile.dart`'s `_findIntermediateWidget` assertion in 3.44 to name it. The defect had shipped since each surface was written | ✅ **Closed by the 3.44.9 upgrade.** `Material(type: MaterialType.transparency)` between the box and the tile: it paints nothing, so the surfaces are byte-identical (`pro_deposit_settings.png` is pixel-identical across the change) and the fix is provably inert — deliberately, so the SDK's own golden drift stayed attributable. **One of the four was invisible locally**: the deposit-settings switch renders only in a golden, and goldens skip on macOS, so that assertion would have fired first on Linux CI. Swept the rest by rendering every golden under `MYWELI_GOLDEN_LOCAL=1` — zero further sites. The general lesson has no gate yet and is recorded rather than claimed: **an interaction affordance that fails silently is invisible to a suite of static assertions** 🟢 |
| 89 | **The brand is spelled two ways, and the two collide one screen apart** | **44 strings · 2 surfaces · every legal document** | found by running the pro flavour on the iOS simulator (the first-run slice). The pro splash renders « **Myweli** Pro » in `displaySmall` — immediately after the user taps an icon whose `CFBundleDisplayName` is « **MyWeli** Pro ». Same word, two spellings, one screen apart, on the very first frame a salon owner ever sees. The canonical form is **MyWeli**, and it was never ambiguous: the brand lockup SVG draws it, `branding-integration.md` uses it 13:1, web's metadata uses it exclusively, and both app display names in `setup_flavours.rb` are `MyWeli` / `MyWeli Pro`. The product simply never applied its own answer — **24** user-visible strings in `mobile/lib/` said `Myweli`, including `AppConstants.appName` itself. **Why nothing caught it.** §7's microcopy rules govern tone, length and punctuation; **no rule in this document has ever said how the brand is spelled**, so there was nothing for a gate to enforce and nothing for a reviewer to cite. Goldens render a wrong spelling as faithfully as a right one — **and, measured, not one of the 24 strings appears in any golden at all**: regenerating the full set of 46 on Linux after the sweep produced **zero** changed pixels. Every site sits behind a state the sheet does not capture — a toggle that is off (the deposit explainer), an error path, a modal, a subscription flow, an admin screen. The slice was scoped expecting « the pro screens and the admin table will move »; they did not, and that is the more useful fact. And the two surfaces drifted *together*: the refusal sentences §21 row 82 deliberately twinned line-for-line on web and mobile carried the misspelling on **both** sides, so a cross-surface diff agreed with itself | ✅ **Swept, and the scope was larger than it looked.** 24 mobile strings + the web twins of those same sentences — fixing mobile alone would have broken row 82's twinning, making the surfaces disagree *inside an identical sentence*, which is worse than being consistently wrong. **Two sites were judged rather than swept.** `timed_cached_image.dart`'s `User-Agent: 'Myweli/1.0'` is machine-facing and **stays** — changing it splits log history for no reader. The Android notification **channel name** changed and the **id did not**: the backend pairs on `androidChannelId = 'myweli_default'` and the test pins the id, so a renamed channel updates in place rather than creating a second one. Identifiers (`MyweliMonthGrid`, `showMyweliDatePicker` — 82 of them) and comments (39) are out of scope: not user-visible. **The legal documents moved too, by owner decision** — `COMPANY.tradingName` and the CGU / mentions légales / politique de confidentialité prose. That was raised as a question rather than swept, because it is the name that goes on the RCCM and the company is not registered yet; the owner chose to align it. **§17 now states the spelling**, so the next occurrence has a rule to violate 🟢 |
| 90 | **The same widget failed two different ways, and the ugly one was the one users saw** | **1 widget · 2 branches · every seeded salon on the home screen** | found by running the consumer app on the simulator against the **live** backend — the first time in this product's history that has been done. `TimedCachedImage` has a network branch and an `asset:` branch. The network branch has always degraded to a `surface` card with `Icons.image_not_supported`. The asset branch had **no `errorBuilder` at all**, so a missing asset threw and Flutter's `ErrorWidget` rendered in its place: a red box, with the exception text drawn across the card, over the salon's own photo slot. It was live. The backend's seed hands out `asset:assets/images/salon1.jpg`, `salon2.jpg`, `barber1.jpg` and `nails1.jpg` — **four files that have never existed in this repository**, checked against all of git history and not merely the working tree. So the featured row of the consumer home screen rendered four red boxes against a real server, and had since the seed was written. **Why the whole suite was green.** Every widget test, every golden and every screen test runs on `MockData`, which points at `assets/images/providers/*_photo.png` — files that DO exist. The mock and the seed had silently diverged, and the half that carried the coverage was the half that was correct. The API-level funnel smoke talks to this exact server and never renders a pixel, so a string that is a valid `String` and an invalid asset sails through it. **The seam is a string convention across two packages** — the server writes `asset:<path>`, the widget resolves it against the Flutter bundle — and a convention with no compiler and no test is a convention that drifts | ✅ **Both halves closed, with the gate on the side that was wrong.** The seed now points at the four bundled photos (two of which, `beaute_divine` and `barber_shop_pro`, match their salon by name), and `backend/test/seed_assets_test.dart` **stats every `asset:` the seed emits against the app's tree** — watched red against the real defect, reporting `['assets/images/barber1.jpg']`. The durable fix is the widget's, though: the asset branch gained the `errorBuilder`, and both branches now render **one shared `_unavailable()`** instead of two ideas of failure. That is what stops the *next* stale path being user-visible, since a seed gate cannot cover an asset renamed out of `pubspec.yaml`. Pinned by `timed_cached_image_test.dart`, watched red (`Found 0 widgets with icon…`), with a companion test that a bundled asset still paints — otherwise "swallow everything into the placeholder" would pass. **Verified live**: the same screen, the same unfixed server, red boxes before and the placeholder after 🟢 |
