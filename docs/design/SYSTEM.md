...
| 37 | **The Android release build produces no APK** (ROADMAP Part 6, NFR-PERF-002) | **every release build** | A9 set out to record the first APK size, because NFR-PERF-002 sets **<30 MB** and PR-0 shipped it with **no measured number anywhere and no CI job checking it** — unfalsifiable since the day it was written. Adding the job found something bigger: `flutter build apk --release --target-platform android-arm64` runs Gradle to completion (~500 s, fonts tree-shaken, `assembleRelease` finishes) and then fails with *"Gradle build failed to produce an .apk file. It's likely that this file was generated under …/mobile/build, but the tool couldn't find it."* Reproduced **twice**, with and without `--analyze-size`, so the flag is not the trigger. **Not caused by A9** — the slice touches nothing under `mobile/android/` (only `pubspec.yaml`), `android/build.gradle.kts` has been unchanged since PR #146, and **no CI job has ever built the APK** (A9's was the first, which is why nobody knew). The likely suspect is that file's `newBuildDir` redirect versus where the tool looks, but that is a diagnosis to prove, not to assume. The job was **removed** rather than shipped red — a permanently-failing check trains people to ignore CI — and returns with the size gate once a release build works. **The app is deployed but not launched; this is a launch blocker, not a design-system row** | *needs its own slice — Android release build* |
