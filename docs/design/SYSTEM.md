...
| 54 | Web's tab strips have mobile's defect and web's gate cannot see them | **2 surfaces** | mobile's three clipping `TabBar`s are mirrored on web as hand-rolled `<button>` strips — `RendezVousClient.tsx:308` (the same four `LIST_TABS` labels as `appointment_list_screen`) and `AccountClient.tsx:269` (the same three as `my_bookings_screen`). Both are plain `flex` with **no `flex-wrap` and no `overflow-x`**, so a `<button>` flex item's `min-width: auto` pushes the page sideways rather than clipping. **And `type-overflow.spec.ts` runs `PUBLIC_ROUTES` only** — `/pro/*` and `/mon-compte` are outside its matrix, so nothing measures either. Note the divergence the parity claim hides: web's *earnings* twin (`RevenusClient.tsx:95`, which calls itself "the app's earnings_screen, web-adapted") solved the same problem with `flex-wrap` on a chip row, while mobile now scrolls. Computed at a 375px viewport, not measured in a browser — a strong lead, not a fact | *new — a web slice; extend the overflow matrix first* |
