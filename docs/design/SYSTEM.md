...
| 40 | `earnings_screen` — a clipped tab label, an untokenised card, a bare empty state (§4, §12, §13) | **3** | invisible until A10 photographed the screen for the first time. The `TabBar` is **not `isScrollable`** with four tabs at 390 px, so « Aujourd'hui » renders **truncated** — French copy cut off in the product. The total card is a bare `Container(color: AppColors.secondary)`, not a surface token: no radius, no elevation, and it reads as an unstyled band. The empty state is `Center(Text('Aucune transaction'))` rather than the shared `EmptyState`, so it has no icon, no title and no action — the four-states contract met in name only. **And a fourth, which only appeared once the screen had data:** `initState` loads with **no date filter** (`:33`) while `_loadEarningsForTab` runs only `onTap`, so the « Aujourd'hui » tab renders *every* transaction until the user touches a tab — the golden shows « dimanche 1 mars 2026 » under it, with the clock frozen to 11 March. These four are recorded, not fixed: A10 is the clock, and this screen needs its own slice. **The functional bug behind them was fixed here** — see row 42 — because a golden of it would have pinned it | *new — needs its own slice* |
