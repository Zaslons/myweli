...
| 37 | The APK budget has never been measured (ROADMAP Part 6) | **no number, no gate** | NFR-PERF-002 sets **<30 MB** and PR-0 shipped it, but **no measured size is recorded anywhere in the repo and no CI job checked it** — so the budget has been unfalsifiable since it was written. A9 both adds weight (`flutter_localizations` ≈ 2.3 MB of generated Dart, and `supportedLocales` does **not** tree-shake it — the delegate is a runtime switch over every locale) and adds the missing job: `Mobile — APK size`, arm64 release, **report-only** until a baseline exists, because a threshold added before the first measurement fails PRs on a pre-existing condition. It could not be measured locally — this machine has no Android SDK at all | *report-only; promote to blocking once the first number is on the record* |
