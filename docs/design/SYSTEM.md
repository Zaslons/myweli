...
- **A value the user reads as one token, and a control's label, may not break
  ...
  - **May**: a heading, a sentence, a salon name. « Salon Ex/cellence » is ugly;
    it is not a defect of the same kind, and §21 row 62 keeps it open.
...
| 41 | « 1 prestation(s) » — the parenthetical plural (§17) | **1 confirmed, unswept** | the journal's timeline row prints `1 prestation(s)`, and the mock's client name renders as the literal placeholder « Client ». Both surfaced in `pro_journal_day`, the first golden of that screen. The `(s)` form is an English habit French does not use — it should be « 1 prestation » / « 2 prestations », i.e. `Intl.plural`, which A9 wired the CLDR data for and no call site uses. §17.1 gates the ellipsis and the apostrophe; it says nothing about plurals, so nothing caught it. One occurrence is confirmed; the app has not been swept for others | *new — a §17.1 rule + a sweep* |
...
| 54 | Web's tab strips have mobile's defect and web's gate cannot see them | **5 strips** | mobile's three clipping `TabBar`s are mirrored on web as hand-rolled `<button>` strips — `RendezVousClient.tsx:308` (the same four `LIST_TABS` labels as `appointment_list_screen`) and `AccountClient.tsx:269` (the same three as `my_bookings_screen`). Both are plain `flex` with **no `flex-wrap` and no `overflow-x`**, so a `<button>` flex item's `min-width: auto` pushes the page sideways rather than clipping. **And `type-overflow.spec.ts` runs `PUBLIC_ROUTES` only** — `/pro/*` and `/mon-compte` are outside its matrix, so nothing measures either. Note the divergence the parity claim hides: web's *earnings* twin (`RevenusClient.tsx:95`, which calls itself "the app's earnings_screen, web-adapted") solved the same problem with `flex-wrap` on a chip row, while mobile now scrolls. Computed at a 375px viewport, not measured in a browser — a strong lead, not a fact | *new — a web slice; extend the overflow matrix first* ⚠️ **Under-counted, corrected by C8d's review:** the census found two and there are **five** — `CatalogueClient.tsx:106`, `MediasClient.tsx:65` and `RendezVousClient.tsx:174` carry the identical `flex gap-s border-b border-divider` strip with no `flex-wrap` and no `overflow-x`, and none of the three is on a `PUBLIC_ROUTES` path either. The count was taken by reading, not by grepping the class string; `grep -rn "flex gap-s border-b border-divider" web/components` returns all five in one line. |
...
| 62 | At 200% text, several labels break mid-word — and the gate permits it | ~~4 measured~~ → **2 fixed, 2 open** | found by looking at C7's first 2× pictures, which is the whole argument for §20.1's review rule. « Salon Ex/cellence » on the salon header · « Prom/o W… » on the home story cards · « Appel/er » on the salon page's call button · and a DATE broken as « 13/03/20 » / « 26 » on `AppointmentCard`. Flutter breaks inside a word when the box is narrower than the word, and **nothing forbids it**: `expectNoUndeclaredTruncation` deliberately permits wrapping (a heading that wraps is correct — that is C5's whole fix), and an overflow never fires because wrapping is how the layout succeeds. So this is the horizontal analogue of the gap §13.3 had before A11 — a real rule with no expression. A date and a button label are the two that clearly must never break; a heading arguably may. **C8 wrote the rule and fixed the two that are unarguable.** §13.3 now says a value read as one token — a date, a price, a phone number — and **a control's label** may not break inside a word; a heading may. `AppointmentCard`'s date row was a `Row` of two `Expanded`s, so the date got half a card and split at 2×; it is a `Wrap` of two intrinsic halves now. The salon page's « Appeler » is one word that cannot wrap its way out of 81dp when it wants 105, so the action bar **stacks** above 1.3× rather than splitting the word — a text-scale branch, not a §10 breakpoint. Both mutation-proven: reverting either reddens `expectNoMidWordBreak` alone, at 2× only, with *« Appeler » … longest word*. **The two headings stay open** — « Salon Ex/cellence » and « Prom/o W… » are a proper noun and a promo title, and whether a heading may break mid-word is a design decision this slice does not make | ✅ **A11 C8** for the two · *open for the two headings* **C8f: both open ones are device-confirmed, and an earlier draft of this sentence got it wrong.** On a 360×780pt iPhone at `accessibility-large` (≈1.95×) the consumer home renders « Prom/o W… » on its story card **and** the salon header renders « Salon Ex / cellence » — both exactly as C7's pictures recorded them. The draft claimed the header one was resolved by C5's `textScaledBound`; opening the salon showed it is not, which is why the claim was checked before it shipped. `textScaledBound` gives the header room to be two lines; it does not choose where the line breaks. Tally unchanged: **two fixed, two open**. |
...
| 71 | The §5 spacing pin cannot see a compound name | **2 raw literals** | the sweep's regex is `\b(?:run)?[Ss]pacing: \d`, and `\b` cannot fire inside `crossAxisSpacing` — so `crossAxisSpacing: 12` and `mainAxisSpacing: 12` on the pro dashboard's grid were invisible to a sweep §20 calls complete. Both happened to equal `spacingSM` and were converted in passing (A12). **The magnitude was wrong too, and the adversarial review measured it:** a proper `(cross|main)AxisSpacing:\s*[0-9]` sweep returns **six**, and the four beyond the dashboard's are `10` — a value that is not a token at all (the scale is 4/8/12/16/24/32/48/64). Two sit in `screens/provider/features/`, which the pin excludes as V2/V3; **two live ones remain in its own corpus** (`pro_photos_screen.dart:141-142`, `mock_image_picker_sheet.dart:46-47`). They are left rather than converted, deliberately: 10 is between `spacingS` and `spacingSM`, so every conversion moves a photo grid, and that belongs with the pin that would catch it. **The pin gap itself is open**: any `*Spacing:` compound is still unswept | *new — widen the pattern* |
