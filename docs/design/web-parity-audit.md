...
| ~~**G4**~~ ✅ | Account `/mon-compte` | **Closed (M8.3):** **rebook** ("Réserver à nouveau") + **laisser un avis** (on completed bookings) + **favoris** (account section + provider-page heart). Still app-side: profile-edit (consumer), notif-prefs, data-export, deposit-screenshot submit. Spec: [web-m8-3-account.md](web-m8-3-account.md). | ~~Medium~~ done |
...
3. **M8 — consumer parity pass:**
   ...
   - **M8.3 ✅ G4** account: rebook + laisser un avis + favoris ([web-m8-3-account.md](web-m8-3-account.md)).
...
**✅ All gaps closed (2026-06-28).** G1–G4 done; the web (public SEO · consumer ·
pro) is at flow-parity with both apps. Deliberate app-side items remain (deposit-
screenshot upload, account deletion/data-export, consumer profile-edit, per-staff
hours, before/after drag-slider) — flagged, not gaps.
