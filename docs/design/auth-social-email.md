...
- **`GoogleIdTokenVerifier`** — fetches & **caches Google's JWKS** (`https://www.googleapis.com/oauth2/v3/certs`, honour `Cache-Control`), verifies **RS256 signature**, `iss ∈ {accounts.google.com, https://accounts.google.com}`, `aud ∈` our **allowlist** of OAuth client IDs (web + android + ios), `exp`/`iat` within skew, `email_verified == true`. Returns `(sub, email, name, picture)` or a typed failure. **Nonce rule (relaxed for Google only, 2026-07-03):** the iOS Google Sign-In SDK (AppAuth) mints and self-validates its OWN `nonce`, so iOS tokens carry a nonce claim we never issued — the verifier **ignores an unsolicited nonce claim** for Google (a caller-presented nonce must still match). Apple stays strict (`rejectUnsolicitedNonce`): there WE issue the nonce, and a token with a nonce claim but no caller nonce is rejected (replay defence intact).
...
- **`AUTH_METHODS` fail-fast is opt-in**: prod boot only *requires* per-method config (`GOOGLE_CLIENT_IDS` etc.) when `AUTH_METHODS` is set explicitly — an unset legacy deploy keeps booting, with the new endpoints failing closed (503 `auth_not_configured`). Zero-risk merge; activation = put the values in Secret Manager, reference them from `infra/gcp/service.yaml`, and deploy.
