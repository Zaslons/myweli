...
| **Status** | **Built** (2026-07-03) — decisions: phone REQUIRED at registration · pro phone-OTP gated in this slice · Apple seam included |
...
- **`pro_login_screen`** → mirrors the consumer `LoginScreen` (lockup, « Espace Pro », Google + Apple (iOS) + email→code), with `provider_not_found` → CTA « Créer un compte » → register screen. No mandatory-phone step (registration already requires it).
- **`pro_register_screen`** → the same pair, « S’inscrire avec … ». Its Apple button shipped as a **dead stub** (`onPressed: () {}`): visible, tappable, and silently doing nothing. Only the *client* was missing — `routes/auth/provider/register.dart` has handled an Apple identity since this slice (`identityToken` → `AppleIdTokenVerifier` → `authProvider: 'apple'`, `appleSub`). Now wired through `registerProviderWithApple`, which splices the credential half of `signInProviderWithApple` (raw nonce to the backend, sha256 to Apple) onto the body half of `registerProviderWithGoogle`.
