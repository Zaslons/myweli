...
- ⚠️ **Pro — payouts of collected deposits** (FR-PRO-PAYOUT-001) — **superseded by the no-custody decision (PRD OQ-1).** The `/pro/payouts` screen + `ProPayoutServiceInterface` were built (#21) assuming Myweli holds deposits; under the V1 direct-payment model Myweli holds nothing, so there's nothing to pay out. **To retire / flag-hide** (reinstate only if escrow via an aggregator is added post-incorporation).
