...
- 🟡 **B3a — repository seam:** `ProvidersRepository` + `AuthRepository` are now **interfaces** with `InMemory*` impls, built in one composition root (`backend/lib/src/dependencies.dart`) and provided to routes via middleware. Makes the Postgres swap (**B3b**) a one-place change. Pure refactor — 33 backend tests green, no contract change. *(B3b adds the Postgres impls + SQL migrations + a CI Postgres service, where the real DB is exercised in CI.)*
