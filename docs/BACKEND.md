...
| T51 | Draft salon leak | **I** — an unfinished salon (no photos, no prices) appears publicly | `status='draft'` at creation; discovery `query()` excludes drafts (both impls) so `/providers`, landings and the sitemap never see them; `by-slug` 404s drafts. Pro-own surfaces resolve by account and keep working. **Booking, amended by row 82:** a CLIENT booking is refused — now with its own code, `provider_not_published`, because sharing `provider_suspended` left every surface unable to say which state it was in and all four fell through to a generic apology. The salon's **own** manual booking is **allowed**: T54 already promises that a billing unpublish (`status → draft`) leaves « journal/bookings/export » working, and refusing only appointments while hours, blocked dates and services stay editable was arbitrary — so this row's old mitigation, « booking refuses them like suspended », was contradicting T54 and is withdrawn. No exposure is added: the write is ownership-gated by `Cap.journalManageAll` and scoped to the salon's own journal, and an account erased under T53 has every token revoked, so nobody can authenticate into the `draft` salon it leaves behind. **Known gap, deliberately open:** `GET /providers/{id}` still serves `draft` (and `by-slug` still serves `suspended`) — closing them is blocked on four mobile pro surfaces that read their own salon through the public route, incl. the owner's pre-publish preview. Tracked as Decision C in docs/design/salon-state-and-refusals.md. | Implemented (pro-salon-lifecycle; booking amended by row 82 — detail-read gap open) |
