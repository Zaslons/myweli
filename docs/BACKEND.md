...
| T5 | `/me`, bookings | **T/E** — act on another user's data | Principal from token `sub`; `/me` self-scoped; consumer appointments scoped/owned (403). **Provider reads/transitions** require `role=provider` **and** the account's linked `providerId`: `GET /appointments` returns only that salon's bookings (unlinked → 403), a transition's appointment `providerId` must match (cross-salon → 403), and the catalogue/availability/**dashboard**/**earnings**/**manual-booking** reads+writes (`/providers/{id}/…`) check `account.providerId == {id}` (→ 403). | Implemented (B2 /me; consumer + pro appointments; B-cat; B-dash; B-earn; B-manual) |
...
| T11 | Booking / reschedule (consumer **and pro**) | **T** — overbooking / book a closed/past/taken slot / move another party's booking | App-level slot-engine validation → `slot_unavailable` (409), **plus** a Postgres **partial unique index** on `(provider_id, appointment_date)` for non-cancelled statuses → atomic exact-start collision rejection under concurrency. Reschedule is **role-aware** and ownership-scoped: a `user` may only move their own booking (by `userId`), a `provider` only its own salon's (by linked `providerId`) — cross-party → 403; both go through the same slot guard. Duration-**overlap** exclusion (btree_gist) pending. | Implemented (app + DB exact-start; consumer + pro reschedule); overlap pending |
| T12 | Catalogue mgmt (`/providers/{id}/services`, `/availability`, `/gallery`) | **T/E** — edit another salon's catalogue / hours / photos | `role=provider` **and** ownership: the account's linked `providerId` must equal `{id}` (cross-salon / unlinked → 403). Boundary validation (name/price/duration/time-windows → 400; gallery = list of non-empty URL strings, ≤20, ≤2048 chars); server sets `id`/`providerId`; a disabled service (`active=false`) is rejected by booking. **Gallery** stores only URLs (no bytes through the API); once the upload pipeline issues URLs (Cloudflare R2), the gallery `PUT` will allowlist server-issued origins (anti-SSRF/hotlink) — follow-up with [pro-image-upload-pipeline.md](design/pro-image-upload-pipeline.md). | Implemented (B-cat; B-gallery URL list); origin-allowlist pending the upload slice |
