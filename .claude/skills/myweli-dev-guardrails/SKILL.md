...
- **[docs/design/SYSTEM.md](../../../docs/design/SYSTEM.md)** — **the design system; for any UI/design work.** Identity (incl. **brand black `primary` vs ink `textPrimary`** — text is never `primary`), **tokens** (`AppColors`/`AppTextStyles`/`AppTheme` — never literal colors/sizes/durations), the component catalog, the **four-states** contract, **accessibility** (≥4.5:1 · **≥48px tap targets** · 200% text scale · `Semantics`/tooltips · focus), **forms** (errors belong to fields, never toasts), feedback & destructive actions, microcopy, the **enforcement gates**, and **§21 — the register of what the code still violates**. **Read it (and the part's design spec) BEFORE designing or building any screen.** *(Supersedes `DESIGN-STANDARDS.md`.)*
...
- **[docs/WEB.md](../../../docs/WEB.md)** + **[docs/design/WEB-SYSTEM.md](../../../docs/design/WEB-SYSTEM.md)** — **for any work in `web/`** (the Next.js web surface): architecture, the OpenAPI-typed client, rendering rules, httpOnly-cookie auth + CORS, SEO/AEO/GEO, **feature parity with the apps**, the install-the-app push, CWV budgets, testing, and the web PR Definition of Done.
...
**Step 0 (always, before designing or building any UI): check the design system.** Re-read [docs/design/SYSTEM.md](../../../docs/design/SYSTEM.md) (identity, tokens, components, four states, **a11y**, forms, feedback) **and** the part's `docs/design/<part>.md` spec, and confirm the work fits them. Design *to* the system — reuse `AppColors`/`AppTextStyles`/`AppTheme` and the shared components; **never invent a color, size, duration, or one-off pattern.** If the system lacks what you need, **add it to SYSTEM.md first** — a literal in a widget is never the answer. This is not optional and not after-the-fact — it precedes the UX plan below. (Memory: `check-design-standards-first`.)
...
- **After a feature ships, add a roadmap entry** — one new file in
  `docs/roadmap/entries/` (`YYYY-MM-DD-slug.md`), never an edit to a shared
  list. `docs/ROADMAP.md` §1.8 is closed history; `./tool/roadmap-log.sh`
  renders the log newest-first (see memory `roadmap-status-refresh`).
