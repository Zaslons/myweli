...
**Step 0 (always, before designing or building any UI): check the standards.** Re-read [docs/design/DESIGN-STANDARDS.md](../../../docs/design/DESIGN-STANDARDS.md) (identity, tokens, components, UX rules) **and** the part's `docs/design/<part>.md` spec, and confirm the work fits them. Design *to* the system — reuse `AppColors`/`AppTextStyles`/`AppTheme` and the shared components; never invent a color, size, or one-off pattern. This is not optional and not after-the-fact — it precedes the UX plan below. (Memory: `check-design-standards-first`.)
...
- [ ] **Design tokens:** no literal colors/sizes — `AppColors`/`AppTextStyles`/`AppTheme` only; shared components reused. Run the consistency sweep ([DESIGN-STANDARDS.md](../../../docs/design/DESIGN-STANDARDS.md) §6) and don't grow the count.
