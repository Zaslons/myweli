...
/// allowlisted to the PRO purposes — `gallery` (default), `kyc`, or `logo`
/// (the salon's brand mark, docs/design/salon-logo.md); `deposit` stays
/// consumer-only on the consumer BFF (docs/design/web-pro-kyc.md §3).
export async function POST(req: NextRequest) {
  const { contentType, purpose } = await req.json().catch(() => ({}));
  if (
    !contentType ||
    (purpose != null && !['gallery', 'kyc', 'logo'].includes(purpose))
  ) {
