...
///
/// axe-core over 13 real routes (public + consumer + pro, stub-seeded), plus
...
test('pro routes are axe-clean', async ({ page }) => {
  ...
  for (const route of ['/pro', '/pro/rendez-vous', '/pro/equipe', '/pro/clients']) {
    await page.goto(route);
    await page.waitForLoadState('networkidle');
    await expectNoViolations(page, route);
  }
...
test('a VISIBLE toast is axe-clean', async ({ page }) => {
  ...
  // Deterministic trigger: a PAST time in the manual-booking dialog enables
  // submit, and create() answers with the error toast (no stub-data
  // dependency — the vacuous-guard lesson from tap-targets.spec).
  await page.getByRole('button', { name: 'Nouveau rendez-vous' }).click();
  await expect(page.getByRole('dialog')).toBeVisible();
  await page.getByRole('dialog').getByRole('checkbox').first().check();
  await page.getByLabel('Rechercher ou nommer le client').fill('Client de test');
  await page.getByLabel('Date du rendez-vous').fill('2020-01-01');
  await page.getByLabel('Heure du rendez-vous').fill('10:00');
  await page
    .getByRole('dialog')
