...
# 10-second window, all 15 land inside one minute, and the 11th meets the app
# limiter rather than the edge. An HTML 429 here is therefore reported as a
# FAIL naming the edge, not waved through as "some 429".
#
...
if [[ "${RUN_CRON:-0}" == "1" ]]; then
  ...
  if gcloud scheduler jobs run myweli-reminders --location="${REGION}" --project="${PROJECT}" >/dev/null 2>&1; then
    ...
    if [[ -z "${AFTER}" || "${AFTER}" == "${BEFORE}" ]]; then
      ...
      # `status.code` is written only on failure; an empty value is success.
      STATE=$(gcloud scheduler jobs describe myweli-reminders --location="${REGION}" \
        --project="${PROJECT}" --format='value(status.code)' 2>/dev/null || true)
      if [[ -z "${STATE}" || "${STATE}" == "0" ]]; then
        ok "myweli-reminders ran at ${AFTER} and the Scheduler recorded success"
      else
        FAIL "myweli-reminders ran at ${AFTER} with status.code=${STATE} — the OIDC call through the Worker was refused"
      fi
    fi
  else
...
# ---------------------------------------------------------------------------
# The vacuity floor. Without the cron, the checks above number 11; a run that
# performed fewer than 8 has lost a section to a parse error or a dead
