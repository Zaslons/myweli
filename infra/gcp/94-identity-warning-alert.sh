...
cat > /tmp/policy-identity-unavailable.json <<JSON
{
  ...
  "conditions": [{
    ...
    "conditionMatchedLog": {
      "filter": "resource.type=\\"cloud_run_revision\\" AND ${SERVICES} AND textPayload:\\"rate_limit_unavailable bucket=\\""
    }
  }],
  "alertStrategy": {
